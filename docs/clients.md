...
Ubuntu 18.04 users can install the <a href="https://packages.ubuntu.com/search?keywords=network-manager-l2tp-gnome" target="_blank">network-manager-l2tp-gnome</a> package, then configure the IPsec/L2TP VPN client using the GUI (Settings -> Network -> VPN). Older Ubuntu versions (e.g. 16.04) may require adding the `nm-l2tp` PPA. Read more <a href="https://medium.com/@hkdb/ubuntu-16-04-connecting-to-l2tp-over-ipsec-via-network-manager-204b5d475721" target="_blank">here</a>.

### Other Linux

See [Configure Linux VPN clients using the command line](#configure-linux-vpn-clients-using-the-command-line).
